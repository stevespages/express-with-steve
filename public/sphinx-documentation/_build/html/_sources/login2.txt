...
Next create app.js (``vim app.js``) and write the code:
...
Next test the registration code with `cURL`:

.. code::

   steve@Dell ~ $ curl -d "username=Steve&password=secret" -X POST http://localhost:3001/register
   steve@Dell ~ $

Next check the user has been entered into the `users` table:
...
.. code::
   ...
   Enter ".help" for usage hints.
   sqlite> SELECT * FROM users;
   1|Steve|$2b$10$dOHTICZC0TwvHnUgmKr6AuHJD.GR6PmrTes7BATGe.EEcjC3jPvfG
   sqlite>

Next check the user with a series of `cURL` requests. There are now a large number of requests we would like to test on our app. If we ran these individually it would get time consuming especially if we had to change the code in app.js and then run them again. So, we can put the `cURL` requests in a file called `curlrequests.sh` and then run the contents of the file from the command line.

`curlrequests.sh` looks like this:

.. code::

   curl....

We alter the permissions on `curlrequests.sh` so we can run it:

.. code::

   steve@Dell ~/my-app $ chmod +x curlrequests.sh

Next we run the commands in `curlrequest.sh`

.. code::

   steve@Dell ~/my-app $ ./curlrequests.sh

Finally here is the output from the http requests made to our app:
